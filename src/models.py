...
def garnet_ae(size=0, latent_dim=8, quant_size=0, pruning=False):
    ...
    # model definition
    encoder = GarNet(4, 16, 2, simplified=True, collapse='mean', input_format='xn',
    ...
    decoder = GarNet(4, 16*3, 2, simplified=True, collapse='mean', input_format='xn',
               output_activation='linear', name='garnet_decoder2', quantize_transforms=False)([encoder, n]) if quant_size <= 0 \
    ...
    # bound eta and phi
    # decoder[:,:,1] = np.pi*tf.math.tanh(decoder[:,:,1])
    # decoder[:,:,0] = 3.29*tf.math.tanh(decoder[:,:,0])
